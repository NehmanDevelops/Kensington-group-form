...
export default async function handler(req, res) {
  ...
  // TEMP: add a test traveller row WITHOUT ticking Ready to Book, so the tick
  // can be a separate real Smartsheet change (remove after use).
  if (norm(body.__addUnreadyTestTraveller) && body.__groupId) {
    ...
    put('Last Name', norm(body.__last) || 'ReadyToBookTest2');
  ...
  // TEMP: tick Ready to Book on a specific row (remove after use).
  ...
  // TEMP: check a row's current Amgine Status / Itinerary ID (remove after use).
