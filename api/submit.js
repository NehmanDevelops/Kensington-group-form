export default async function handler(req, res) {
  ...
  try {
    ...
    // The Group ID column is auto-generated by Smartsheet automation AFTER a
    // row lands on the intake sheet — the client never sends one — so the
    // dedup check below (which matches on Group ID) is a no-op for brand-new
    // submissions. That let retries (slow network, double-click before the
    // button visually disables, refresh-and-resubmit) create duplicate rows
    // on the intake sheet, the master sheet, AND the agent sheet. Guard here
    // instead: if a row with the same Company Name + Event Manager Email was
    // created in the last 3 minutes, treat this as a repeat and skip the
    // insert entirely (report success — the original submission already went
    // through).
    const INTAKE_SHEET_ID = '3569349083221892';
    const INTAKE_COMPANY_COL = 1061015075983236;
    const INTAKE_EM_EMAIL_COL = 3312814889668484;
    const dupCompany = String((smartsheetPayload.cells || []).find(c => c.columnId === INTAKE_COMPANY_COL)?.value || '').trim().toLowerCase();
    const dupEmail = String((smartsheetPayload.cells || []).find(c => c.columnId === INTAKE_EM_EMAIL_COL)?.value || '').trim().toLowerCase();
    if (dupCompany && dupEmail) {
      try {
        const recentRes = await fetch(`https://api.smartsheet.com/2.0/sheets/${INTAKE_SHEET_ID}?columnIds=${INTAKE_COMPANY_COL},${INTAKE_EM_EMAIL_COL}`, {
          headers: { 'Authorization': `Bearer ${process.env.SMARTSHEET_API_TOKEN}` },
        });
        if (recentRes.ok) {
          const recentData = await recentRes.json();
          const cutoff = Date.now() - 3 * 60 * 1000;
          const isDupe = (recentData.rows || []).some(row => {
            const createdAt = row.createdAt ? new Date(row.createdAt).getTime() : 0;
            if (createdAt < cutoff) return false;
            const c = row.cells?.find(x => x.columnId === INTAKE_COMPANY_COL);
            const e = row.cells?.find(x => x.columnId === INTAKE_EM_EMAIL_COL);
            const cVal = String(c?.value || '').trim().toLowerCase();
            const eVal = String(e?.value || '').trim().toLowerCase();
            return cVal === dupCompany && eVal === dupEmail;
          });
          if (isDupe) {
            console.log(`Skipped duplicate submission — "${dupCompany}" / "${dupEmail}" already submitted within the last 3 minutes.`);
            return res.status(200).json({ success: true, deduped: true });
          }
        }
      } catch (dupErr) {
        console.error('Duplicate-submission check failed, proceeding with insert:', dupErr.message);
      }
    }
