...
export default async function handler(req, res) {
  ...
  // TEMP: full-pipeline smoke test helpers (remove after use, no schema changes).
  const __ssHelper = () => {
    const TOKEN = process.env.SMARTSHEET_API_TOKEN;
    return (path, opts = {}) => fetch(`https://api.smartsheet.com/2.0${path}`, {
      ...opts, headers: { Authorization: `Bearer ${TOKEN}`, 'Content-Type': 'application/json', ...opts.headers },
    });
  };
  if (norm(body.__pipelineCreateRow)) {
    const ss = __ssHelper();
    const sheet = await (await ss(`/sheets/${GROUPS}?pageSize=1`)).json();
    const idByTitle = {};
    for (const c of sheet.columns || []) idByTitle[c.title.trim().toLowerCase()] = c.id;
    const cells = [];
    const set = (title, value) => { if (idByTitle[title]) cells.push({ columnId: idByTitle[title], value }); };
    set('group id', norm(body.__groupId));
    set('name', norm(body.__name));
    set('address line 1', '1 Test Way');
    set('city', 'Test City');
    set('province/state', 'ON');
    set('postal code', 'A1A1A1');
    set('country', 'CA');
    ...
    return res.status(200).json({ ok: r.ok, rowId, raw: r.ok ? undefined : j });
  }
  if (norm(body.__pipelineTickCol) && body.__rowId) {
    const ss = __ssHelper();
    const sheet = await (await ss(`/sheets/${GROUPS}?pageSize=1`)).json();
    const col = (sheet.columns || []).find(c => c.title.trim().toLowerCase() === norm(body.__pipelineTickCol));
    if (!col) return res.status(200).json({ ok: false, error: `column "${body.__pipelineTickCol}" not found` });
  ...
  if (norm(body.__pipelineCheckRow) && body.__rowId) {
    ...
    const idByTitle = {};
    for (const c of sheet.columns || []) idByTitle[c.title.trim().toLowerCase()] = c.id;
    const row = (sheet.rows || []).find(r => String(r.id) === norm(body.__rowId));
    ...
    return res.status(200).json({
      ok: true, branchGuid: val('amgine branch guid'), whiteLabelUrl: val('white label url'),
      enableWhiteLabel: val('enable white label'), whiteLabelStatus: val('white label status'),
      amgineOnboarded: val('amgine onboarded'),
    });
  }
