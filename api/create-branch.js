...
export default async function handler(req, res) {
  ...
  // TEMP DEBUG (2026-08-26): find all branches by name + full config, for duplicate-branch triage. Remove after.
  if (req.query?.testFindAllByName) {
    ...
    for (const m of matches) {
      const dr = await fetch(branchUrl(m.id), { headers: { Authorization: `Bearer ${token}` } });
      ...
      out.push({
        ...
        flightBookingPcc: detail?.flightBookingPcc?.identifier, profilePcc: detail?.profilePcc?.identifier,
        addressLine1: detail?.addressLine1, city: detail?.city,
      });
    }
