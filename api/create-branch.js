...
export default async function handler(req, res) {
  ...
  // TEMP DEBUG (2026-08-26): verify 3rd test branch's queue/connector. Remove after.
  if (req.query?.testFindByGuid3) {
    const q = req.query.testFindByGuid3.toLowerCase();
    const token = await getToken();
    const amg = (url, payload, method = 'POST') => fetch(url, {
    ...
    return res.status(200).json({ id: match.id, name: match.name, city: match.city, country: match.country,
      ...
      flightBookingPcc: detail?.flightBookingPcc, currentConnector: currentConnector?.description || null });
  }
